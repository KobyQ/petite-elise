...
const AddProduct: React.FC<AddProductProps> = ({
  ...
  const [formData, setFormData] = useState({
    ...
    // Step 2: Images (just files, not uploaded yet)
    images: [] as ImageFile[],
  ...
  const renderStep2 = () => (
    <div className="space-y-4">
      ...
      {formData.images.length > 0 && (
        <div>
          <label className="block font-semibold mb-2">Selected Images ({formData.images.length})</label>
  ...
  const renderStep3 = () => (
    <div className="space-y-4">
      ...
      {Object.keys(formData.details).length > 0 && (
        <div>
          <label className="block font-semibold mb-2">Added Details</label>
  ...
  return (
    <Dialog open={isOpen} onOpenChange={async (open) => {
      if (!open) {
        // On cancel/close, clean up any uploaded images to avoid orphan files
        // The cleanup is now handled after successful submission or on cancel
        setIsOpen(false);
        resetForm();
      }
    }}>
          ...
          <div className="flex justify-between pt-4 border-t">
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                variant="outline"
                onClick={async () => {
                  // On cancel, clean up any uploaded images
                  for (const image of formData.images) {
                    if (image.preview) {
                      URL.revokeObjectURL(image.preview);
                    }
