...
const DevelopmentalPlayGroupRegistration = () => {
  ...
  const fetchPricingForPrograms = async (programs: string[]) => {
    try {
      // For Developmental Playgroup, use monthly pricing
      ...
      try {
        ...
        if (values.hasSibling === "true") {
          ...
          for (const child of allChildren) {
            const pricing = await fetchPricingForPrograms(["Developmental Playgroup"])
            if (!pricing) {
              toast.error(`Unable to fetch pricing for ${child.childName}. Please try again.`)
              return
            }
          ...
          const registrationData = {
            ...
            program_type: "Developmental Playgroup",
          }
          ...
          const response = await fetch("https://api.paystack.co/transaction/initialize", {
            ...
            body: JSON.stringify({
              ...
              callback_url: `${window.location.origin}/developmental-playgroup-registration/verify`,
            }),
          })
              ...
              <div className="space-y-2 text-sm">
                {siblings.map((child, index) => (
                  <div key={index} className="flex justify-between text-blue-700">
                    ...
                    <span>Developmental Playgroup</span>
                  </div>
                ))}
    ...
    >
      <div className="max-w-5xl mx-auto px-2 md:px-8">
        <div className="text-center mb-10">
          <h2 className="text-3xl md:text-4xl font-extrabold">Developmental Playgroup Registration</h2>
                ...
                <div className="space-y-1 text-sm">
                  {siblings.map((child, index) => (
                    <div key={index} className="flex justify-between text-green-700">
                      <span>{child.childName}</span>
                      <span className="font-medium">Developmental Playgroup</span>
                    </div>
                  ))}
                </div>
              </div>
