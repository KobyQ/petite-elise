    ...
    onSubmit: async (values, { setSubmitting, setFieldValue }) => {
      try {
        ...
        if (values.hasSibling === "true") {
          ...
          setFieldValue("saturdayClubSchedule", "")
          ...
          const childrenWithoutSchedules = allChildren.filter(child => !child.saturdayClubSchedule)
          ...
          for (const child of allChildren) {
            if (!child.saturdayClubSchedule) continue
            const pricing = await fetchPricingForSchedule(child.saturdayClubSchedule)
            if (!pricing) {
              ...
              <div className="space-y-2 text-sm">
                {siblings.map((child, index) => (
                  <div key={index} className="flex justify-between text-blue-700">
                    ...
                    <span>{child.saturdayClubSchedule}</span>
                  </div>
                ))}
                <div className="flex justify-between text-blue-700">
                  <span>{formik.values.childName}:</span>
                  <span>{formik.values.saturdayClubSchedule}</span>
                </div>
              </div>
            )}
                ...
                <div className="space-y-1 text-sm">
                  {siblings.map((child, index) => (
                    <div key={index} className="flex justify-between text-green-700">
                      <span>{child.childName}</span>
                      <span className="font-medium">{child.saturdayClubSchedule}</span>
                    </div>
                  ))}
                </div>
              </div>
