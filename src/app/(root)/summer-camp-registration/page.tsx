    ...
    onSubmit: async (values, { setSubmitting, setFieldValue }) => {
      try {
        ...
        if (values.hasSibling === "true") {
          ...
          setFieldValue("summerCampSchedule", "")
          ...
          const childrenWithoutSchedules = allChildren.filter(child => !child.summerCampSchedule)
          ...
          for (const child of allChildren) {
            if (!child.summerCampSchedule) continue
            const pricing = await fetchPricingForSchedule(child.summerCampSchedule)
            if (!pricing) {
              ...
              <div className="space-y-2 text-sm">
                {siblings.map((child, index) => (
                  <div key={index} className="flex justify-between text-blue-700">
                    ...
                    <span>{child.summerCampSchedule}</span>
                  </div>
                ))}
                <div className="flex justify-between text-blue-700">
                  <span>{formik.values.childName}:</span>
                  <span>{formik.values.summerCampSchedule}</span>
                </div>
              </div>
            )}
                ...
                <div className="space-y-1 text-sm">
                  {siblings.map((child, index) => (
                    <div key={index} className="flex justify-between text-green-700">
                      <span>{child.childName}</span>
                      <span className="font-medium">{child.summerCampSchedule}</span>
                    </div>
                  ))}
                </div>
              </div>
